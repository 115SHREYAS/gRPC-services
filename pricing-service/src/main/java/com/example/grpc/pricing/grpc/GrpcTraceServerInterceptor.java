...
public class GrpcTraceServerInterceptor implements ServerInterceptor {
    ...
    public <ReqT, RespT> ServerCall.Listener<ReqT> interceptCall(
        ...
        String caller = headers.get(CALLER_SERVICE);
        log.info("pricing-service gRPC method={} correlationId={} caller={}",
                call.getMethodDescriptor().getFullMethodName(),
                correlationId,
                caller);

        Context context = Context.current()
                .withValue(GrpcTraceContext.CORRELATION_ID, correlationId)
                .withValue(GrpcTraceContext.CALLER, caller);
